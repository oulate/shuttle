...
def get_config(is_local):
    ...
    config['password'] = to_bytes(config.get('password', b'vps123456'))
    config['method'] = to_str(config.get('method', 'chacha20-ietf'))
    config['protocol'] = to_str(config.get('protocol', 'auth_chain_a'))
    config['protocol_param'] = to_str(config.get('protocol_param', 'itunes.apple.com'))
